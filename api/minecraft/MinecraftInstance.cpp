...
#include "launch/CreateServerResourcePacksFolder.h"
#include "launch/ExtractNatives.h"
#include "launch/PrintInstanceInfo.h"
#include "launch/LauncherPartLaunch.h"
#include "launch/ModMinecraftJar.h"
...
QMap<QString, QString> MinecraftInstance::createCensorFilterFromSession(std::shared_ptr<Minecraft::AuthSession> session)
{
	if(!session)
	{
...
{
	auto mcSession = std::dynamic_pointer_cast<Minecraft::AuthSession>(session);
	if(!mcSession)
	{
		return nullptr;
	}
	...
	if(mcSession->status != Minecraft::AuthSession::PlayableOffline)
	{
	...
	{
		auto step = std::make_shared<PrintInstanceInfo>(pptr);
		process->appendStep(step);
	}
...

	{
		...
		auto step = createMainLaunchStep(pptr, mcSession);
		process->appendStep(step);
	}
	...
	if (mcSession)
	{
		process->setCensorFilter(createCensorFilterFromSession(mcSession));
	}
