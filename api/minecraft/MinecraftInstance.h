...
class MULTIMC_MINECRAFT_EXPORT MinecraftInstance: public BaseInstance
{
public:
	...
	virtual QString createLaunchScript(std::shared_ptr<Minecraft::AuthSession> session) = 0;
	...
	virtual QStringList processMinecraftArgs(std::shared_ptr<Minecraft::AuthSession> account) const = 0;

	virtual JavaVersion getJavaVersion() const;

protected:
	QMap<QString, QString> createCensorFilterFromSession(std::shared_ptr<Minecraft::AuthSession> session);
	virtual QStringList validLaunchMethods() = 0;
	virtual QString launchMethod();
	virtual std::shared_ptr<LaunchStep> createMainLaunchStep(LaunchTask *parent, std::shared_ptr<Minecraft::AuthSession> session) = 0;
private:
