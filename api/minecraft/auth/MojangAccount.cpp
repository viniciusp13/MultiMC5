...
std::shared_ptr<YggdrasilTask> MojangAccount::login(AuthSessionPtr session, QString password)
{
	...
	auto mcSession = std::dynamic_pointer_cast<AuthSession>(session);
	if(!mcSession)
	{
		return nullptr;
	}
	...
	if (accountStatus() == NotVerified && password.isEmpty())
	{
		if (session)
		{
			mcSession->status = AuthSession::RequiresPassword;
			fillSession(session);
		}
		return nullptr;
	}
...
void MojangAccount::authSucceeded()
{
	...
	if (mcSession)
	{
		mcSession->status = mcSession->wants_online ? AuthSession::PlayableOnline : AuthSession::PlayableOffline;
		fillSession(session);
		mcSession->auth_server_online = true;
	}
	m_currentTask.reset();
	emit changed();
}
...
void MojangAccount::authFailed(QString reason)
{
	...
	if (m_currentTask->state() == YggdrasilTask::STATE_FAILED_SOFT)
	{
		if (session)
		{
			mcSession->status = accountStatus() == Verified ? AuthSession::PlayableOffline
														  : AuthSession::RequiresPassword;
			mcSession->auth_server_online = false;
			fillSession(session);
		}
	}
	else
...
void MojangAccount::fillSession(AuthSessionPtr session)
{
	...
	mcSession->username = m_username;
	// volatile auth token
	mcSession->access_token = m_accessToken;
	// the semi-permanent client token
	mcSession->client_token = m_clientToken;
	if (currentProfile())
	{
		// profile name
		mcSession->player_name = currentProfile()->name;
		// profile ID
		mcSession->uuid = currentProfile()->id;
		// 'legacy' or 'mojang', depending on account type
		mcSession->user_type = currentProfile()->legacy ? "legacy" : "mojang";
		if (!mcSession->access_token.isEmpty())
		{
			mcSession->session = "token:" + m_accessToken + ":" + m_profiles[m_currentProfile].id;
		}
		else
		{
			mcSession->session = "-";
