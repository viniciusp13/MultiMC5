...
class MULTIMC_MINECRAFT_EXPORT OneSixInstance : public MinecraftInstance
{
	Q_OBJECT
public:
	...
	virtual QString createLaunchScript(std::shared_ptr<Minecraft::AuthSession> session) override;
	...
	QStringList processMinecraftArgs(std::shared_ptr<Minecraft::AuthSession> account) const override;

protected:
	std::shared_ptr<LaunchStep> createMainLaunchStep(LaunchTask *parent, std::shared_ptr<Minecraft::AuthSession> session) override;
