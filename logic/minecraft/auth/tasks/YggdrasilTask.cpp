...
void YggdrasilTask::processReply()
{
	...
	try
	{
		const QJsonObject response = Json::requireObject(Json::requireDocument(replyData));
...
void YggdrasilTask::processError(const QJsonObject &responseData)
{
	...
	try
	{
		changeState(STATE_FAILED_HARD, requireString(responseData, "errorMessage"));
	}
	catch (...)
	{
...
YggdrasilTask::State YggdrasilTask::state() const
{
	return m_state;
}
MojangAccount *YggdrasilTask::account() const
{
	return m_account;
}
MojangAuthSessionPtr YggdrasilTask::session() const
{
	return m_session;
}
